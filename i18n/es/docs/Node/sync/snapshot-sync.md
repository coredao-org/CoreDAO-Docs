...
On the Core network, you can sync your node to the latest state of the Core blockchain in several ways. En este documento, lo guiamos a través del proceso de sincronización de nodos en la red Core.
...
:::note
Note that the recommended method for syncing the testnet node is to sync from the genesis block, as currently the snapshot is not available. Puede descargar el binario más reciente para testnet desde [aquí](https://github.com/coredao-org/core-chain/releases).
:::
...
- Si no se proporciona la marca **--syncmode**, el modo de sincronización predeterminado dependerá del estado de la carpeta de datos. It will be **snap** mode if you sync from the genesis or **full** mode if you start from a snapshot.